...
let baseTime = parseInt(helpers.readFile("last-production.txt"), 10);
let fileCount = 0,
...
function sendSMS(contact) {
  return aws
    .sendSMS({
      ...
      Message:
        "Mehab.net has been updated. Give the CDN about 10 minutes to refresh\r\n\r\n"
    })
    .then(result => {
...
function uploadFiles(files) {
  for (let index = 0; index < files.length; index++) {
    ...
    if (fs.existsSync(file)) {
      let fileTime = fs.statSync(file).mtimeMs;
...
Promise.all(uploads).then(results => {
  ...
  helpers.createFile("last-production.txt", current_time, true);
});
